...
//
// ИСПРАВЛЕНИЕ: инициализация пула запускается в фоне (не блокирует старт сервера).
// Ранее блокирующий await в instrumentation вызывал 502/белый экран при старте.

export async function runServerInit() {
  const g = globalThis;

  // Запускаем в фоне — НЕ блокируем старт HTTP-сервера
  _warmupAndMigrate(g).catch(e =>
    console.warn('[ServerInit] Background init error:', e.message)
  );

  console.log('[ServerInit] Background initialization started');
}

async function _warmupAndMigrate(g) {
  // ── 1. Прогрев пула ──────────────────────────────────────────────────────
  try {
  ...
  // Ждём готовности пула (макс 15 секунд) — теперь в фоне, не блокирует HTTP
  for (let i = 0; i < 75; i++) {
  ...
  if (!g._pgPool || !g._pgReady) {
    console.warn('[ServerInit] Pool not ready after 15s, skipping migration');
    return;
  }
