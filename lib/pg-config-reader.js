...
//
// Порядок приоритетов:
//  1. DATABASE_URL — env-переменная (heroku-style)
//  2. PG_HOST      — отдельные PG_* env-переменные
//  3. data/pg-config.json — сохраняется через UI и должна перебивать старый pg.env
//  4. data/pg-env.json    — резервная копия
//  5. store.json[__pg_config__] — fallback для переживания git-деплоев (используется в store.js)
//  6. pg.env       — bootstrap/fallback из репозитория, если ещё ничего не сохраняли через UI
...
export function readPgConfig({ includeStoreJson = false, ensureDirFn = null } = {}) {
  ...
  // ── 2. PG_HOST env ──────────────────────────────────────────────────────
  if (_hasUsableHostConfig(process.env))
    return { ..._buildHostConfig(process.env), source: 'env_host' };

  // ── 3. data/pg-config.json (сохраняется через UI) ───────────────────────
  try {
  ...
  // ── 5. store.json[__pg_config__] (опциональный fallback для store.js) ───
  if (includeStoreJson) {
    try {
  ...
  // ── 6. pg.env в корне репозитория — только как bootstrap/fallback ──────
  try {
    if (fs.existsSync(PG_ENV_FILE)) {
      const vars = parseEnvFile(fs.readFileSync(PG_ENV_FILE, 'utf8'));
      if (_hasUsableHostConfig(vars) || vars.DATABASE_URL) {
...
function _hasUsableHostConfig(vars) {
  return !!(vars?.PG_HOST && vars?.PG_DATABASE && vars?.PG_USER);
}
